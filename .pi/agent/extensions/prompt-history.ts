  ...
  pi.registerShortcut('ctrl+r', {
    ...
    handler: async (ctx: ExtensionContext) => {
      ...
      if (selected) {
        ...
        // setEditorText updates the editor buffer but does not request a render,
        // so the recalled text would only appear on the next keystroke.
        // setStatus with an unused key is a no-op that does request a render.
        ctx.ui.setStatus('prompt-history:refresh', undefined);
      }
    }
  });
}
