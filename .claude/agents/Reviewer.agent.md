...
argument-hint: What should be reviewed (diff, PR, file, or directory)
tools: Read, Grep, Glob, Bash(git diff:*), Bash(git log:*), Bash(git show:*), Bash(gh pr view:*), Bash(gh pr diff:*), WebFetch
agents: [Explore]
color: red
---
...
2. **Context** — read the surrounding code of each change; for broad impact checks (callers, usages), delegate to the Explore agent
